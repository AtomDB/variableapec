...
def set_up(Z, z1, Te, dens, process, delta_r, transition, transition_2, all_levels, \
    ...
    ax1.set_xlabel('Temperature in keV', fontsize=18) 
    ax1.set_ylabel('Emissivity', fontsize=18)
    ...
    for key in emiss.keys():
        if isinstance(key, numpy.int32) == True:
            if (int(key) < z1):
                ion = pyatomdb.atomic.int_to_roman(int(key)) 
                ax1.semilogy(emiss['Te'], emiss[key], label='Ionization of '+element+' '+ion)
            if (int(key) == z1):
                ion = pyatomdb.atomic.int_to_roman(int(key)) 
                ax1.semilogy(emiss['Te'], emiss[key], label='Excitation from '+element+' '+ion, linewidth=3)
            if (isinstance(key, numpy.int32) == True) & (int(key) > z1):
                ion = pyatomdb.atomic.int_to_roman(int(key)) 
                ax1.semilogy(emiss['Te'], emiss[key], label='Recombination of '+element+' '+ion)
...
#wrapper_check_sensitivity still needs to be finished
def wrapper_check_sensitivity(Z, z1, Te, dens, process, delta_r, \
    ...
    print('\n\n\ntransition list is', transition_list, "\n\n\n")
...
def check_sensitivity(Z, z1, Te, dens, process, delta_r, transition, transition_2=None, \
    ...
    if transition_2 is None:    #check sensitivity for a single transition
        ...
        #plot sensitivity for both transitions at same time (puts on one graph) 
        ...
        plot_line_diagnostics(inputs, line_diagnostics_1)
        plot_line_diagnostics(inputs_2, line_diagnostics_2)
